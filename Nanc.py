...
mabase="miniprojet_db"
app.config['SQLALCHEMY_DATABASE_URI']="postgresql://postgres:{}@localhost:5432/{}".format(motdepasse,mabase)
...
db.session.commit()
###############################################################
#verifie que le nombre de livre de la categorie =0 , c'est une fonction intermediaire du procesus
###############################################################
...
def return_err():
    return jsonify({
            'success': False,
            'error':404,
            'message': "not found verify the existence of the object ",
        })
###############################################################
#CORS
###############################################################
